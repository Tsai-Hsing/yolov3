...
class test_detector:
    ...
    def test(cls, img, path, graph, session):
        try:
            ...
            fin.write(data)
            fin.close()
            resultstr = cls.readcmd('./darknet detector test "Tsai-Hsing.Lu@quantatw.com" ' + path + '"obj.data" ' + path + '"yolov3.cfg" ' + path + modelname + '".weights" "original.' + img.format.lower() + '"  2>&1 | tee -a ./temp.log')
            #print('\n\n\n\n\n\n\n')
